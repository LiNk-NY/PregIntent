...
ignoreVars <- c("ResponseID", "StartDate", "EndDate", "Finished",
    ...
    "Q1.1b", paste0("Q3.17_", 1:4))
## Remove Q3.17a, the select all that apply question that was intended
## as single select

keepL1 <- !names(pregint) %in% ignoreVars
keepL2 <- names(pregint) %in%
    unique(c(codebookCode[["recodeName"]], recodebook[["hname"]]))
keepvars <- keepL1 & keepL2

keepnames <- names(pregint)[keepvars]
```

Filter variables that have been recoded:

```{r}
pregintDESC <- pregint[, keepvars]
